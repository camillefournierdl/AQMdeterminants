...
nonOECDFUA <- subset(nonOECDFUA, GDP15_SM != 0) # also remove cities that do not have GDP per capita 

nonOECDFUA$isMonitor <- as.factor(ifelse((nonOECDFUA$numberMonitor) > 0, 1, 0))
nonOECDFUA$isNonUSMonitor <- as.factor(ifelse((nonOECDFUA$numberMonitor - nonOECDFUA$numberUSEmbassyMonitors) > 0, 1, 0))
...
write.csv(nonOECDFUA_adjusted %>% select(ID_HDC_G0),
          "outputData/FUA_nsf_merged_monitors_subset.csv",
          row.names = F) # save for consistency in future plots

write.csv(nonOECDFUA %>% select(ID_HDC_G0),
          "outputData/FUA_nsf_merged_monitors_nonOECD.csv",
          row.names = F) # save for consistency in future plots
...
nonOECDFUA$isUSMonitor <- as.factor(ifelse((nonOECDFUA$numberUSEmbassyMonitors) > 0, 1, 0))
nonOECDFUA$isOnlyUSMonitor <- as.factor(ifelse((nonOECDFUA$isMonitor == 1 & ((as.numeric(nonOECDFUA$numberMonitors) - as.numeric(nonOECDFUA$numberUSEmbassyMonitors)) == 0)), 1, 0))
...
nonOECDFUA$conflict_cumulative_intensity_22_num <- as.numeric(ifelse(nonOECDFUA$conflict_cumulative_intensity_22 == "war", 1,0))
...
test_results <- lapply(variables, function(var) perform_tests(nonOECDFUA, var))
...
# democracy (important one) significant *** for non democracies
...
nonDem <- subset(nonOECDFUA, vdem_bin == "non-democracy")
...
dem <- subset(nonOECDFUA, vdem_bin == "democracy")
...
nonOECDFUA_adjusted$conflict_cumulative_intensity_22_num <- as.numeric(ifelse(nonOECDFUA_adjusted$conflict_cumulative_intensity_22 == "war", 1,0))

nonOECDFUA_adjusted$GDP15_SMpc <- nonOECDFUA_adjusted$GDP15_SM/nonOECDFUA_adjusted$P15
...
# Apply the tests to each variable
test_results <- lapply(variables, function(var) perform_tests(nonOECDFUA_adjusted, var))
...
# CAREFUL THO with wilcox test ONLY
...
##########
permTS(as.numeric(nonOECDFUA_adjusted %>% filter(vdem_bin == "non-democracy") %>% pull(pm25VanD20002016) %>% na.omit()), as.numeric(nonOECDFUA_adjusted %>% filter(vdem_bin == "democracy") %>% pull(pm25VanD20002016) %>% na.omit()), method = "exact.mc", control = permControl(nmc = 1000))

nonDem_adj <- subset(nonOECDFUA_adjusted, vdem_bin == "non-democracy")
...
dem_adj <- subset(nonOECDFUA_adjusted, vdem_bin == "democracy")
...
# First Figure is a map of points in FUAs for different databases
...
#load list of FUAs to keep
df_subset_fua <- read.csv('outputData/FUA_nsf_merged_monitors_subset.csv', sep = ',', allowEscapes = T) 

df_nonOECD_fua <- read.csv('outputData/FUA_nsf_merged_monitors_nonOECD.csv', sep = ',', allowEscapes = T) 

listFUA <- unique(df_subset_fua$ID_HDC_G0)
listFUA_nonOECD <- unique(df_nonOECD_fua$ID_HDC_G0)

FUAs_sp <- st_read("data/FUA_fixed_geom.gpkg") # spatially explicit FUAs

listFUA_nonOECD <- unique(df_nonOECD_fua$ID_HDC_G0)

FUAs_nonOECD <- subset(FUAs_sp, ID_HDC_G0 %in% listFUA_nonOECD)

points_within_fuas <- st_join(data3Sub_sf, FUAs_nonOECD, join = st_intersects) %>% subset(., !is.na(ID_HDC_G0))
...
ggsave(plot = p, filename = paste('plotsMain/nbMonitorsDatabasesinFua.png', sep = ""),
...
map_plot <- ggplot() +
  ...
  geom_sf(data = mollPointdf, aes(color = color), size = 0.8, alpha = 0.3) +
...
ggsave(plot = map_plot, filename = paste('plotsMain/monitorsFuaPoints.png', sep = ""),
...
ggsave(plot = map_plot, filename = paste("plotsExtra/maps/supplementaryFigure8.png", sep = ""),
       dpi=600, width = 18, height = 12, units='cm')
...
ggsave(plot = p, filename = paste("plotsExtra/maps/supplementaryFigure8legend.png", sep = ""),
...
ggsave(plot = legend_grob, filename = paste('plotsExtra/maps/countriesMonitors_legend3.png', sep = ""),
...
ggsave(plot = map_plot, filename = paste('plotsExtra/maps/countriesMonitors.png', sep = ""),
       dpi=600, width = 18, height = 12, units='cm')

########## Now we remove points that don't fall in FUAs Supplementary Figure 7 ########## ########## ########## 
...
FUAs_nonOECD_terra <- vect(FUAs_nonOECD)

# points_within_fuas_ST <- st_join(data3Sub_sf, FUAs_nonOECD, join = st_intersects) %>% subset(., !is.na(ID_HDC_G0))

points_within_fuas_terra <- terra::intersect(data3Sub_terra, FUAs_nonOECD_terra) 
...
# data3Sub_chloro <- vect(points_within_fuas, geom=c("longitude", "latitude"), crs="+proj=longlat +datum=WGS84")

# Define the buffer distance (e.g., 500 meters)
buffer_distance <- 500  # Adjust this value based on your needs

# Create a buffer around the points
buffered_points <- buffer(points_within_fuas_terra, width = buffer_distance)
...
ggsave(plot = key_plot, filename = paste('plotsExtra/maps/countriesMonitorsinFUA_legend2.png', sep = ""),
...
ggsave(plot = map_plot, filename = paste('plotsExtra/maps/countriesMonitorsinFUA.png', sep = ""),
       dpi=600, width = 18, height = 12, units='cm')
...
#load FUA dataset (filter after subset)

FUAs <- read.csv('outputData/FUA_nsf_merged_monitorsOpenAQWAQI.csv', sep = ',', allowEscapes = T)

FUA_gridded <- read.csv("outputData/FUA_nsf_monitors_mergedGrid.csv")
FUAs <- merge(FUAs, FUA_gridded, by = "ID_HDC_G0")
...
FUAs$INDCHNOTH <- factor(ifelse(FUAs$CTR_MN_NM == "India", "India",
                         ifelse(FUAs$CTR_MN_NM == "China", "China",
                                ifelse(is.na(FUAs$CTR_MN_NM), NA, "Other"))), levels = c("Other", "India", "China"))

FUAs$numberMonitorsNonUS <- FUAs$numberMonitors - FUAs$numberUSEmbassyMonitors

FUAs$isMonitorNonUSE <- ifelse(FUAs$numberMonitorsNonUS > 0, "Yes", "No")

FUAs <- subset(FUAs, log(GDP15_SM+1) != 0)

FUAs_subset <- subset(FUAs, ID_HDC_G0 %in% listFUA)
FUAs_nonOECD <- subset(FUAs, ID_HDC_G0 %in% listFUA_nonOECD)

FUAs_subset$GDP15_SM_PC <- FUAs_subset$GDP15_SM/FUAs_subset$P15

lines_df <- FUAs_subset %>% 
...
p <- FUAs_subset %>% ggplot(aes(x = log(GDP15_SM+1), fill = isMonitorNonUSE))+
...
ggsave(plot = p, filename = paste('plots/FUAsGDPAQM.png', sep = ""),
       dpi=600, width = 18, height = 15, units='cm')


p <- FUAs_subset %>% ggplot(aes(x = log(P15), fill = isMonitorNonUSE))+
  geom_histogram(bins = 50)+
...
ggsave(plot = p, filename = paste('plots/FUAsPopAQM.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')


p <- FUAs_subset %>% ggplot(aes(x = mean_edi, fill = isMonitorNonUSE))+
...
ggsave(plot = p, filename = paste('plotsMain/FUAsDemoAQM.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')



p <- FUAs_subset %>% ggplot(aes(x = log(GDP15_SM_PC), fill = isMonitorNonUSE))+
  geom_histogram(bins = 50)+
...
ggsave(plot = p, filename = paste('plotsMain/FUAsGDPpcAQM.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')


lines_df <- FUAs_nonOECD %>% 
  group_by(isMonitorNonUSE, vdem_bin) %>% 
  summarize(meanPollution = mean(pm25VanD20002016, na.rm=T))

p <- FUAs_nonOECD %>% ggplot(aes(x = pm25VanD20002016, fill = isMonitorNonUSE))+
...
ggsave(plot = p, filename = paste('plots/FUAsDemPollutionAQM_fullDF.png', sep = ""),
...
p <- FUAs_subset %>% ggplot(aes(x = pm25VanD20002016, fill = isMonitorNonUSE))+
  geom_histogram(bins = 80)+
  geom_vline(data = lines_df, aes(xintercept = meanPollution, color = isMonitorNonUSE), linetype = "dashed", size = 1) +
  geom_text(data = lines_df, aes(x = meanPollution, y = 170,
                                 label = round(meanPollution, 1), color = isMonitorNonUSE),
...
x_range <- range(FUAs_subset$pm25VanD20002016, na.rm = TRUE)

# Split the dataset into two subsets
democracy_data <- FUAs_subset %>% filter(vdem_bin == "democracy")
non_democracy_data <- FUAs_subset %>% filter(vdem_bin == "non-democracy")
...
ggsave(plot = p_democracy, filename = paste('plotsMain/FUAsDemPollutionAQM_subset_demo.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')

ggsave(plot = p_non_democracy, filename = paste('plotsMain/FUAsDemPollutionAQM_subset_nonDemo.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')
...
p <- FUAs_nonOECD %>% ggplot(aes(x = pm25VanD20002016, fill = INDCHNOTH))+
...
ggsave(plot = p, filename = paste('plotsMain/FUAsPollutionINDCHNOTH.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')


country_FUAcounts <- FUAs_nonOECD %>%
  group_by(CTR_MN_NM) %>%
...
p <- ggplot(country_FUAcounts, aes(x = reorder(CTR_MN_NM, -total_count), y = Count, fill = Condition)) +
...
ggsave(plot = p, filename = paste('plots/FUAnbCountry30_2.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')


country_FUAcounts <- FUAs_subset %>%
  group_by(CTR_MN_NM) %>%
  summarise(total_count = n(),
            count_with_condition = sum(numberMonitors > 1),
            count_without_condition = sum(numberMonitors <= 1)) %>%
...
ggsave(plot = p, filename = paste('plots/FUAnbCountry30_subset_2.png', sep = ""),
       dpi=600, width = 12, height = 8, units='cm')
...
points_within_fuas <- subset(points_within_fuas, type %in% c("Reference Grade", "Sensor"))

# Calculate the total number of monitors for each country
totalMonitorsPerCountry <- points_within_fuas %>%
  group_by(CTR_MN_NM) %>%
...
statsCountry <- points_within_fuas %>%
...
totalFUAPopPerCountry <- as.data.frame(FUAs_nonOECD) %>%
  group_by(CTR_MN_NM) %>%
...
statsCountry <- merge(statsCountry, totalFUAPopPerCountry, by = "CTR_MN_NM")
...
nonOECDFUA_rdy <- nonOECDFUA
nonOECDFUA_rdy$GDP15_SMpc <- log(nonOECDFUA_rdy$GDP15_SMpc+1)
nonOECDFUA_rdy$P15 <- log(nonOECDFUA_rdy$P15)
nonOECDFUA_rdy$conflict_cumulative_intensity_22 <- as.numeric(as.factor(nonOECDFUA_rdy$conflict_cumulative_intensity_22))-1
nonOECDFUA_rdy$vdem_bin <- as.numeric(as.factor(nonOECDFUA_rdy$vdem_bin))-1
nonOECDFUA_rdy$capital <- as.numeric(as.factor(nonOECDFUA_rdy$capital))-1
nonOECDFUA_rdy$dummyCHN <- as.numeric(as.factor(nonOECDFUA_rdy$dummyCHN))-1
nonOECDFUA_rdy$dummyIND <- as.numeric(as.factor(nonOECDFUA_rdy$dummyIND))-1

nonOECDFUA_adjusted_rdy <- nonOECDFUA_adjusted
nonOECDFUA_adjusted_rdy$GDP15_SMpc <- log(nonOECDFUA_adjusted_rdy$GDP15_SMpc+1)
nonOECDFUA_adjusted_rdy$P15 <- log(nonOECDFUA_adjusted_rdy$P15)
nonOECDFUA_adjusted_rdy$conflict_cumulative_intensity_22 <- as.numeric(as.factor(nonOECDFUA_adjusted_rdy$conflict_cumulative_intensity_22))-1
nonOECDFUA_adjusted_rdy$vdem_bin <- as.numeric(as.factor(nonOECDFUA_adjusted_rdy$vdem_bin))-1
nonOECDFUA_adjusted_rdy$capital <- as.numeric(as.factor(nonOECDFUA_adjusted_rdy$capital))-1
nonOECDFUA_adjusted_rdy$dummyCHN <- as.numeric(as.factor(nonOECDFUA_adjusted_rdy$dummyCHN))-1
nonOECDFUA_adjusted_rdy$dummyIND <- as.numeric(as.factor(nonOECDFUA_adjusted_rdy$dummyIND))-1
...
inside(mv, {
  dataset <- branch(dataset_choice,
             data1 = nonOECDFUA_rdy,
             data2 = nonOECDFUA_adjusted_rdy)
}
       )
...
p1 <- ggplot(subset(coef_dataModels, group == "Key Variables" & dataset_choice == "nonOECDFUA_adjusted_rdy" & monitorType == "everything_noEmbassy"), aes(y = Level, x = Mean_Predicted)) +
  geom_point(size = 2) +
  geom_linerange(aes(xmin = lowCI, xmax = highCI, group = Scenario, alpha = sign), linewidth = 1.5) +
  facet_wrap(~ Variable, scales = "free_y", nrow = 6, strip.position = "left") +
  labs(x = "Mean Predicted Value", y = "", title = "Key Variables") +
...
p2 <- ggplot(subset(coef_dataModels, group == "Control Variables" & dataset_choice == "nonOECDFUA_adjusted_rdy" & monitorType == "everything_noEmbassy"), aes(y = Level, x = Mean_Predicted)) +
  geom_point(size = 2) +
  geom_linerange(aes(xmin = lowCI, xmax = highCI, group = Scenario, alpha = sign), linewidth = 1.5) +
  facet_wrap(~ Variable, scales = "free_y", nrow = 6, strip.position = "left") +
  labs(x = "Mean Predicted Value", y = "", title = "Control Variables") +
...
~ log(GDP15_SMpc+1) + pm25VanD20002016 * vdem_bin +
 ...
 log(P15) +
  ...
  +  dummyCHN
  ,
data = nonOECDFUA_adjusted, family = poisson())

AIC(model) # 7286.738

prediction_grid <- expand.grid(
  GDP15_SMpc = mean(nonOECDFUA_adjusted$GDP15_SMpc, na.rm = TRUE), # Using median or mean as appropriate
  vdem_bin = c("1democracy", "0non-democracy"), # Both categories
  pm25VanD20002016 = seq(min(nonOECDFUA_adjusted$pm25VanD20002016, na.rm = TRUE), max(nonOECDFUA_adjusted$pm25VanD20002016, na.rm = TRUE), length.out = 100),
  conflict_cumulative_intensity_22 = "nowar",
  CPI_2012_2022 = mean(nonOECDFUA_adjusted$CPI_2012_2022, na.rm = TRUE),
  P15 = mean(nonOECDFUA_adjusted$P15, na.rm = TRUE),
  capital = 1,
  dummyIND = 0,
  dummyCHN = 0
)
...
p <- prediction_grid %>% ggplot(aes(x = pm25VanD20002016, y = prediction)) +
  ...
  geom_histogram(data = nonOECDFUA_adjusted, aes(y = after_stat(count)/1000), bins = 30, fill = "grey70", alpha = 0.4, position = "identity") +
...
nonOECDFUA <- subset(nonOECDFUA, log(GDP15_SM+1) != 0) # here I subset the countries that are not in the vdem dataset
nonOECDFUA$GDP15_SMpc <- nonOECDFUA$GDP15_SM/nonOECDFUA$P15
...
nonOECDFUA <- subset(nonOECDFUA, GDP15_SM != 0) # here I subset the countries that have GDP = 0

nonOECDFUA$isMonitor <- as.numeric(ifelse((nonOECDFUA$numberMonitor - nonOECDFUA$numberUSEmbassyMonitors) > 0, 1, 0))

# Try with a dummy IND and CHN
nonOECDFUA$dummyCHN <- as.numeric(ifelse(nonOECDFUA$CTR_MN_NM == "China", 1, 0))
nonOECDFUA$dummyIND <- as.numeric(ifelse(nonOECDFUA$CTR_MN_NM == "India", 1, 0))
...
capitals <- subset(nonOECDFUA, capital == T)
...
nonOECDFUA$isMonitor <- as.factor(ifelse((nonOECDFUA$numberMonitor - nonOECDFUA$numberUSEmbassyMonitors) > 0, 1, 0))
nonOECDFUA$numberMonitorNonEmbassy <- nonOECDFUA$numberMonitor - nonOECDFUA$numberUSEmbassyMonitors
...
prediction_grid <- expand.grid(
  GDP15_SMpc = quantile(nonOECDFUA_adjusted$GDP15_SMpc, probs = seq(0, 1, length.out = 9), na.rm = TRUE),
...
gdp_labels <- setNames(paste("Quantile GDP p.c.", (0:8)/0.08), quantile(nonOECDFUA_adjusted$GDP15_SMpc, probs = seq(0, 1, length.out = 9), na.rm = TRUE))
...
dir.create("plotsExtra/subsamplingLogit")
...
ggsave(plot = p, filename = paste("plotsExtra/subsamplingLogit/suppFig6histInteraction.png", sep = ""),
       dpi=600, width = 12, height = 10, units='cm')
...
```{r testing whether different subsets of the IND / CHN subsampling lead to different results in multiverse --- MAYBE NOT NEEDED, PREVIOUS CHUNK IS ENOUGH}
...
# FUA_data$isMonitorAll <- as.factor(ifelse(FUA_data$numberMonitors>0, 1, 0))
# FUA_data$isMonitorWAQIOpenAQ_embassy <- as.factor(ifelse(FUA_data$numberMonitorsWAQIOpenAQ_embassy>0, 1, 0))
# FUA_data$isMonitorWAQIOpenAQ_noEmbassy <- as.factor(ifelse(FUA_data$numberMonitorsWAQIOpenAQ_noEmbassy>0, 1, 0))
# 
# FUA_data$isGovMonitor <- as.factor(ifelse(FUA_data$numberGovMonitors>0, 1, 0))
# FUA_data$isNonGovMonitor <- as.factor(ifelse(FUA_data$numberNonGovMonitors>0, 1, 0))
# FUA_data$isNonUSEMonitor <- as.factor(ifelse((FUA_data$numberMonitors - FUA_data$numberUSEmbassyMonitors)>0, 1, 0))
...
# Plot
# p <- gridPredictionModels %>% ggplot(aes(x = pm25Data, y = prediction, color = vdem_bin)) +
#   geom_line() +
#   scale_color_manual(values = c("#9fcbee", "#d8031c")) +
#   geom_abline(slope = 0, intercept = 1)+
#   geom_ribbon(aes(ymin = lower, ymax = upper, fill = vdem_bin), alpha = 0.2) +
...
#   theme_minimal()+
#   facet_wrap(~ model + dataset_choice + monitorsType + pollutionDataset, scales = "free_y", ncol = 8, strip.position = c("top"))
# 
# ggsave(plot = p, filename = paste('C:/Users/cfournier/work_files/air_quality/majaDataCode/advanced/finalFigures/labelMultiverse.png', sep = ""),
#        dpi=600, width = 25, height = 20, units='cm')
# 
# 
# p <- gridPredictionModels %>% ggplot(aes(x = pm25Data, y = prediction, color = vdem_bin)) +
...
# ggsave(plot = p, filename = paste('C:/Users/cfournier/work_files/air_quality/majaDataCode/advanced/finalFigures/noLabelMultiverse.png', sep = ""),
...
# summaryModels <- expanded %>%
#   mutate(summaryModel = map(.results, "summaryModel")) %>% 
...
# Extract the specific value from each summaryModel
# summaryModelsPValuePollution <- summaryModels %>%
#   mutate(estimate = map(summaryModel, ~ .x[[1]]["pm25Data:vdem_binnon-democracy", "Estimate"]),
#          p_value = map(summaryModel, ~ .x[[1]]["pm25Data:vdem_binnon-democracy", "Pr(>|z|)"]))

# Unnest the list of specific values into a column
# summaryModelsPoisson <- summaryModelsPValuePollution %>%
#   unnest(cols = c(estimate, p_value))
