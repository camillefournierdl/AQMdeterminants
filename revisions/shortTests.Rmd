...
naIncome <- subset(nonOECDFUA, GDP15_SM <= 0) # 314 cities
...
FUAsterra <- vect("data/FUA_fixed_geom.gpkg")

valuesAtFUAs <- terra::extract(gdpKummu, FUAsterra, fun=mean, weights = T, ID = F, na.rm=TRUE)

valuesAtFUAs <- valuesAtFUAs %>% rename(gdppcKummu2015 = gdp_pc_2015)

newFUAs <- cbind(FUAsterra, valuesAtFUAs)

toMerge <- as.data.frame(newFUAs) %>% select(ID_HDC_G0, gdppcKummu2015)

nonOECDFUA <- merge(nonOECDFUA, toMerge, by = "ID_HDC_G0", all.x = T, all.y = F)

# nonOECDFUA <- subset(nonOECDFUA, GDP15_SM != 0) # 

nonOECDFUA$GDP15_SMpc <- nonOECDFUA$GDP15_SM/nonOECDFUA$P15

plot(nonOECDFUA$GDP15_SMpc, nonOECDFUA$gdppcKummu2015)
...
prediction_grid <- expand.grid(
  gdppcKummu2015 = mean(nonOECDFUA_adjusted$gdppcKummu2015, na.rm = TRUE), # Using median or mean as appropriate
  vdem_bin = c("1democracy", "0non-democracy"), # Both categories
  pm25VanD20002016 = seq(min(nonOECDFUA_adjusted$pm25VanD20002016, na.rm = TRUE), max(nonOECDFUA_adjusted$pm25VanD20002016, na.rm = TRUE), length.out = 100),
  conflict_cumulative_intensity_22 = "nowar",
  CPI_2012_2022 = mean(nonOECDFUA_adjusted$CPI_2012_2022, na.rm = TRUE),
...
nonOECDFUA_adjustedM <- merge(nonOECDFUA_adjusted, pollution %>% 
...
fit <- glm(formula = isMonitor
                    ...
                    log(P15)
                    + dummyCHN + dummyIND
                  , binomial(link = "logit"), data = nonOECDFUA_adjustedM)

prediction_grid <- expand.grid(
  GDP15_SMpc = mean(nonOECDFUA_adjustedM$GDP15_SMpc, na.rm = TRUE), # Using median or mean as appropriate
  vdem_bin = c("1democracy", "0non-democracy"), # Both categories
  mean_second_half_pollution = seq(min(nonOECDFUA_adjustedM$mean_second_half_pollution, na.rm = TRUE), max(nonOECDFUA_adjustedM$mean_second_half_pollution, na.rm = TRUE), length.out = 100),
  conflict_cumulative_intensity_22 = "nowar",
  CPI_2012_2022 = mean(nonOECDFUA_adjustedM$CPI_2012_2022, na.rm = TRUE),
  P15 = mean(nonOECDFUA_adjustedM$P15, na.rm = TRUE),
  capital = 1,
  dummyIND = 0,
  dummyCHN = 0
)
...
p <- prediction_grid %>% ggplot(aes(x = mean_second_half_pollution, y = prediction)) +
  geom_line(aes(color = vdem_bin)) +
  geom_histogram(data = nonOECDFUA_adjustedM, aes(y = after_stat(count)/1000), bins = 30, fill = "grey70", alpha = 0.4, position = "identity") +
  scale_color_manual(values = c("#9fcbee", "#d8031c"), guide = "none") +
  geom_abline(slope = 0, intercept = 1, linewidth = 0.4, linetype = "dashed")+
  geom_ribbon(aes(color = vdem_bin, ymin = lower, ymax = upper, fill = vdem_bin), alpha = 0.2, linewidth = 0.2) +
  scale_fill_manual(values = c("#9fcbee", "#d8031c"),
...
nonOECDFUA$dummyCHN <- as.factor(ifelse(nonOECDFUA$CTR_MN_NM == "China", 1, 0))
nonOECDFUA$dummyIND <- as.factor(ifelse(nonOECDFUA$CTR_MN_NM == "India", 1, 0))
nonOECDFUA$dummyETH <- as.factor(ifelse(nonOECDFUA$CTR_MN_NM == "Ethiopia", 1, 0))
